...
class PuntoMapaPage extends StatefulWidget {
  ...
  final String nombre;
  final String imagen;
  const PuntoMapaPage({Key? key, required this.puntos, required this.nombre, required this.imagen}) : super(key: key);
...
class PuntoMapaPageState extends State<PuntoMapaPage> {
  ...
  late AppBar topAppBar = AppBar(
    ...
    title:  Text(widget.nombre, maxLines: 2,),
  );
  ...
  void _onMapCreated(HereMapController hereMapController) {
    hereMapController.mapScene.loadSceneForMapScheme(MapScheme.hybridNight, (MapError? error) {
      if (error == null) {
        ...
        _mapItemsExample?.addPhotoMapMarker(geoCoordinates, 0, 'resources/images/ic_launcher.png', ''+'', '', '');
